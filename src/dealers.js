 ...
 *   GET /api/dealers/:id/policies     — policy events for one dealer (access-checked)
 *
 * Access rules:
...
// ── Policy summary query (shared with admin.js logic) ──────────────────────────

async function queryPolicySummary(env, dealerKey) {
...
export async function handleDealers(request, env, path, method, dealer) {
  ...
  // GET /api/dealers/:id/policies — access-checked policy summary
  const policyMatch = subPath.match(/^\/([a-z0-9-]+)\/policies$/);
  if (policyMatch && method === 'GET') {
    ...
    try {
      const summary = await queryPolicySummary(env, targetDealerId);
      return json({ dealerKey: targetDealerId, ...summary });
    } catch (err) {
      return json({ error: err.message }, 500);
    }
  }
